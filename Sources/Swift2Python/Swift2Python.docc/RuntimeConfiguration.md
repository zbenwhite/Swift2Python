...
You can also set environment variable `SWIFT2PYTHON_LIBRARY` before launching the Swift process:
...
2. environment variable `SWIFT2PYTHON_LIBRARY`.
...
Set environment variable `SWIFT2PYTHON_PYTHONPATH` when your embedded Python code needs project-local modules or vendored packages:

```sh
export SWIFT2PYTHON_PYTHONPATH=/path/to/project/python:/path/to/project/vendor
```

Swift2Python copies this value to environment variable `PYTHONPATH` immediately before `Py_Initialize()`. Use the same path separator Python expects on the target platform.

## Initialization Timing

Python runtime configuration must be set before the first successful initialization. After CPython is initialized, changing environment variables `SWIFT2PYTHON_LIBRARY`, `SWIFT2PYTHON_PYTHONPATH`, or `PYTHONPATH` does not reload the runtime.
